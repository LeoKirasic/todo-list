...
function loadDom() {
      ...
      item.addEventListener('click', (e) => {
        ...
        for (let i = 0; i < projects[index].arrayOfTodos.length; i++) {
          ...
          deleteButton.addEventListener('click', (e) => {
            const todoIndex = projects[index].arrayOfTodos.findIndex(
              (item) =>
                item.info ===
                e.target.closest('#task').textContent.replace('X', '')
            );
  ...
  todoButton.addEventListener('click', () => {
    if (todoTitleInput.value === '' || todoDueDateInput.value === '') {
    ...
    } else if (selectedProject.textContent === 'No project selected!') {
      alert('Please select a project!');
    } else {
      const index = projects.findIndex(
        (item) => item.title === selectedProject.textContent
      );
      ...
      deleteButton.addEventListener('click', (e) => {
        const todoIndex = projects[index].arrayOfTodos.findIndex(
          (item) =>
            item.info === e.target.closest('#task').textContent.replace('X', '')
        );
